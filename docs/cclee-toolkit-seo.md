...
**Enable SEO Enhancer module** 控制整个 SEO Tab 全部功能。
...
<StepBox title="1. 勾选 Enable IndexNow">

在 SEO Tab 勾选 **Enable IndexNow — auto-notify search engines when content is published**。

</StepBox>

<StepBox title="2. 生成或粘贴 Key">

点击 **Generate Key** 自动生成 API Key，或粘贴已有 key。Key 文件自动虚拟托管于 `/{key}.txt`。
...
<StepBox title="1. 创建 Google Service Account">

在 Google Cloud Console 创建 Service Account，启用 Indexing API。

</StepBox>

<StepBox title="2. 授予 Search Console 权限">

在 Google Search Console 中授予该 Service Account "Site owner" 权限。

</StepBox>

<StepBox title="3. 勾选并填写">

勾选 **Enable Google Indexing API — push URLs directly to Google Index**，粘贴 Service Account JSON 到字段中。
...
### Manual Submission — 手动提交单条 URL

无需任何配置，SEO 模块开启时始终可用。

<StepBox title="1. 输入 URL">

滚动到 **Manual Submission** 区域，输入完整 URL（如 `https://yoursite.com/page/`）。

</StepBox>

<StepBox title="2. 选择通道">

勾选发送通道：**IndexNow** 和/或 **Google**。

</StepBox>

<StepBox title="3. 点击 Submit">
...
<StepBox title="1. 勾选 Enable llms.txt">

在 SEO Tab 勾选 **Enable llms.txt — auto-generate a text file for LLM crawlers**。

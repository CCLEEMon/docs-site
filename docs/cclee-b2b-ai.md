...
steps:
  ...
  - name: 上传知识库文档
    text: 上传 PDF、TXT 或 MD 文档，AI 基于文档回答客户问题
  - name: 使用 AI 客服
...
3. **上传知识库**（可选）：上传 PDF/TXT/MD 文档，AI 基于文档回答政策类问题
4. **开始使用**：访客点击气泡即可发起对话
...
AI 从已上传的公司文档中搜索答案，回复关于退换货政策、保修条款、运输说明等问题。支持跨语言问答——中文提问也能匹配英文文档。
...
| 字段 | 说明 |
|------|------|
| Enable Knowledge Base | 开启后 AI 可搜索已上传文档回答问题 |
| 上传文件 | 支持 PDF、TXT、MD 三种格式，单文件最大 10MB |
| 文档列表 | 显示已上传文档的文件名、上传日期、分块数量，可逐个删除 |

<InfoBox variant="info" title="跨语言支持">
知识库文档可以用任意语言编写。客户用中文、英文或其他语言提问时，AI 都能理解并从文档中找到对应答案。推荐文档总量控制在 50KB 以内以获得最佳效果。
</InfoBox>

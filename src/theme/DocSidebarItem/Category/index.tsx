/**
 * Swizzle 自 @theme/DocSidebarItem/Category（Eject）。
 * 与上游唯一差异：一级分类链接内注入 lucide 图标（映射见 sidebarIcons.ts）。
 * 升级 Docusaurus 后需对照上游检查本文件。
 */
...
import {getCategoryIcon} from './sidebarIcons';
...
function DocSidebarItemCategoryCollapsible({
  ...
  const updateCollapsed = (toCollapsed: boolean = !collapsed): void => {
  ...
  // 一级分类注入 lucide 图标；stroke=currentColor，颜色随链接态（muted/hover/active）变化
  const Icon = getCategoryIcon(label, level);
  return (
    <li
      className={clsx(
